...
def _deny(code: ReasonCode, trace_id: str, *, gate: str | None = None) -> MoneyActionResult:
    """`gate`, when given, is a G1-G7 label for `actl_gate_denials_total`
    (§22) -- omitted for the pre-check denials (integrity halt, malformed
    request) that precede G1, since those aren't one of the seven
    numbered gates."""
    if gate is not None:
        metrics.gate_denials_total.labels(gate=gate).inc()
...
async def execute_money_action(
    ...
    with tracing.transaction_span(
        "gate.execute_money_action", trace, mandate_id=req.mandate_id
    ):
        try:
        ...
        if (
            req.amount_minor <= 0
            or not req.mandate_id
            or not req.decision_id
            or not req.quote_id
        ):
...
async def _run_g1_through_g5(
    ...
    with tracing.span("gate.G1_mandate_validity"):
        got = await uow.mandates.get(req.mandate_id)
        if got is None:
            return _deny(ReasonCode.MANDATE_INVALID, trace, gate="G1")
        mandate, status = got
        if status is MandateStatus.REVOKED:
            return _deny(ReasonCode.MANDATE_REVOKED, trace, gate="G1")
        ...
        if not verify_spec_hash(mandate):
            return _deny(ReasonCode.MANDATE_TAMPERED, trace, gate="G1")
        if mandate.spec_hash is None or mandate.signature is None:
            return _deny(ReasonCode.MANDATE_UNSIGNED, trace, gate="G1")
    ...
    with tracing.span("gate.G3_policy_verdict"):
        if decision.verdict != "ALLOW":
            return _deny(decision.reason_codes[0], trace, gate="G3")

    # G4 -- atomic budget reservation. Row lock (inside ledger_reserve)
    # makes concurrent overspend across this mandate impossible.
    with tracing.span("gate.G4_budget_reservation"):
...
async def _run_g6_g7_execute(
    ...
    trace = req.trace_id
    order_id = new_id("ord")
    with tracing.span("gate.G6_G7_idempotency_audit_and_execute", order_id=order_id):
        try:
            ...
            metrics.gate_denials_total.labels(gate="G6_G7").inc()
            return MoneyActionResult(
                verdict="DENY",
                reason_code=ReasonCode.PROVIDER_TERMINAL,
                trace_id=trace,
                duplicate=True,
            )
            ...
            # An unexpected internal failure during G6/G7 still happened
            # *after* S1's reservation already committed -- the same
            # "nothing raises, nothing leaks" contract as the sibling
            # branch above applies regardless of which exception type
            # tripped it. Never self-compensating here would leave a
            # reservation dangling forever on any bug/infrastructure
            # failure this branch wasn't specifically written to name.
            await release_reservation_and_mark_compensated(
                session_factory,
                clock,
                mandate_id=req.mandate_id,

...
async def handle_catalog_query(
    ...
    *,
    ...
    return HandlerResult(type="catalog.query", body=feed.model_dump(mode="json", by_alias=True))
...
async def handle_quote_request(
    ...
    return HandlerResult(type="quote.request", body=quote.model_dump(mode="json", by_alias=True))
...
async def handle_receipt_issue(uow: UnitOfWork, *, order_id: str) -> HandlerOutcome:
    ...
    if order.status != "CAPTURED":
        return HandlerError(
            ReasonCode.ORDER_NOT_SETTLED, f"order {order_id} is not yet settled", retryable=True
        )
    seq_range = await uow.audit_log.get_seq_range_for_order(order_id)
    return HandlerResult(
        type="receipt.issue",

...
async def begin_purchase(
    ...
    with tracing.transaction_span(
        "saga.begin_purchase", req.trace_id, mandate_id=req.mandate_id
    ):
        key = compute_idempotency_key(req.mandate_id, req.intent_hash, req.attempt_no)

        with tracing.span("saga.S1_reserve"):
        ...
        with tracing.span("saga.S2_order"):
        ...
        return _snapshot(
            final, reason_code=None if result.verdict == "ALLOW" else result.reason_code
        )
...
async def complete_purchase(
    ...
    *,
    ...
    trace_id = new_id("trc")
    with tracing.transaction_span("saga.complete_purchase", trace_id, saga_id=saga_id):
        async with UnitOfWork(session_factory) as uow:
        ...
        if current is not None and current[1] is MandateStatus.REVOKED:
            with tracing.span("saga.C2_void_revoked"):
        ...
        with tracing.span("saga.S3_authorize"):
            with tracing.span("provider.fetch_payments", order_id=order_id):
                payments = await provider.fetch_payments(provider_order_id)
            declined = any(
                p.id == provider_payment_id and p.status == "failed" for p in payments
            )
        if declined:
            with tracing.span("saga.C2_void_declined"):
                await void_order_and_release_reservation(
                    session_factory,
                    clock,
                    mandate_id=saga.mandate_id,
        ...
        try:
            with tracing.span("saga.S4_capture"):
                async with UnitOfWork(session_factory) as uow:
            ...
            with tracing.span("saga.C2_void_invalid_signature"):
                await void_order_and_release_reservation(
                    session_factory,
                    clock,
                    mandate_id=saga.mandate_id,
            ...
            with tracing.span("saga.C2_void_capture_failed"):
                await void_order_and_release_reservation(
                    session_factory,
                    clock,
                    mandate_id=saga.mandate_id,
        ...
        with tracing.span("saga.S5_settle"):
            async with UnitOfWork(session_factory) as uow:
                ...
                if not captured:
                    await uow.commit()
                    with tracing.span("saga.C4_C5_refund_and_reverse"):
